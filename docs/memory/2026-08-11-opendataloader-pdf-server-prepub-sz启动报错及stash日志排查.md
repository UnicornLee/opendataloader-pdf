...
3. **Kibana 上看不到日志（真正根因）**：Kibana 上的数据视图（Data View）选错了。stash 写入的索引属于 `logs-jetty-default`（即 jetty 来源），但用户之前选的是 `logs-tomcat-default`，两者底层 Elasticsearch 索引不同，自然查不到 `opendataloader-pdf-server` 的日志。**把数据视图切到 `logs-jetty-default` 后即可看到日志**。
...
- **stash 端问题留给服务端排查**：应用侧验证 `LogstashTcpSocketAppender` 已连接、网络端口通；如果 Kibana 上仍看不到日志，**最终定位为 Kibana 数据视图（Data View）选错**——本服务发出的日志 JSON 带 `type:"jetty_log"`，落在 `logs-jetty-default` 索引；之前用户选的是 `logs-tomcat-default`，自然查不到。详见根因第 3 条。
...
- **保留 `apm-toolkit-logback-1.x` 依赖**：本次未改动该依赖；但后续已在 2026-08-11 的 `2026-08-11-skywalking依赖移除及kibana无日志根因分析.md` 任务中删除（logback-spring.xml 同时移除了 SkyWalking 的 `TraceIdMDCPatternLogbackLayout` 和 `TraceIdJsonProvider` 引用，该依赖失去用途）。
...
- **不深入改动 Logstash 服务端**：应用侧已确认发送成功；Kibana 无日志的实际原因是数据视图选择错误（选了 `logs-tomcat-default` 而非 `logs-jetty-default`），不属于服务端配置问题。
...
- 用户反馈：即使显式以 `--spring.profiles.active=prepub-sz` 启动，Kibana 上仍未看到日志；应用侧发送通道已建立。
- **Kibana 根因已定位**：Kibana 数据视图（Data View）选错了。stash 写入的索引对应 `logs-jetty-default`（因为 JSON 中的 `type:"jetty_log"`），但用户之前选的是 `logs-tomcat-default`。把数据视图切到 `logs-jetty-default` 后即可正常看到 `opendataloader-pdf-server` 的日志。无需修改任何代码或服务端配置。
...
- 查看日志：
  ...
  ```
- 若 Kibana 仍看不到日志，先确认数据视图（Data View）选择是否正确：本服务通过 `LogstashTcpSocketAppender` 发出的日志 JSON 中带 `type:"jetty_log"` 字段，Kibana 端应选 **`logs-jetty-default`**，而不是 `logs-tomcat-default`。
- 若数据视图正确但仍无日志，再按以下顺序排查服务端：
  ...
  4. 检查 Elasticsearch 索引名称及 Kibana 时间范围
  5. 检查 Logstash output 中是否有 `if` 条件把 `type:"jetty_log"` / `program:"opendataloader-pdf-server"` 过滤掉
...
- Logstash / Elasticsearch / Kibana 服务端配置：根因已定位为 Kibana 数据视图选择问题（应选 `logs-jetty-default` 而非 `logs-tomcat-default`），无需改动服务端。
